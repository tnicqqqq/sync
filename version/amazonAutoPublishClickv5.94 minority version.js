// ==UserScript==
// @name         amazonAutoPublishClickv5.94 minority version
// @namespace    only to translate; won't click kd for all situations
// @version      2024-06-26
...
<button id="trimnsavemoban">trimnsavemoban</button>
...
const trimnsavemoban= document.getElementById('trimnsavemoban');
function clickkdbtn(){
    const buttons = document.querySelectorAll('.ivu-btn.ivu-btn-primary');
    for (const button of buttons) {
...
function functrimnsavemoban(){
trimkw();
    setTimeout(()=>{clickkdbtnsave()},1000);
};

observer.observe(body, { attributes: true });
getdesbutton.addEventListener('click', ()=>{getdescription();trimkw()});
trimnsavemoban.addEventListener('click', functrimnsavemoban);
