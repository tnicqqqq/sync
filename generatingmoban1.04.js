// ==UserScript==
// @name         generatingmoban1.04
// @namespace    from testforgeneratingmoban uk and minority version 1.0
// @namespace  do not input
// @namespace  1.02 ad triw as promise;1500 delay before start
// @version      2024-06-26
...
function trimkw(){
    ...
    kwTextareas.forEach(kwTextarea => {
        kwTextarea.value= kwTextarea.value.slice(0, 245);
        kwTextarea.dispatchEvent(event);
    });
    console.log("trimed");resolve();
    },1500)
...
function clicksuggestionitem(inputvalue,suggestionnum,kkinputvalue){
    ...
    if (suggestionItem) {
        ...
        }catch (error){console.log("suggestionItem 点击跳过")}
        //console.log(kkinputvalue);
        //return kkinputvalue;
    } else {
...
function checkElement() {
  return new Promise((resolve, reject) => {
      //bp element
    const elements = Array.from(document.querySelectorAll('div[data-v-27cc2bb9].ivu-col.ivu-col-span-1'));
       document.querySelectorAll('.ivu-collapse-header')[0].click();
                                document.querySelectorAll('.ivu-collapse-header')[2].click();

    if (elements[0].textContent.trim() === '0/500') {
        //alert('Span text content is 0');
        document.querySelectorAll('.ivu-collapse-header')[1].click();
        document.querySelectorAll('.ivu-collapse-header')[3].click();
        reject('Span text content is 0');
      } else {
...
function processAllValues() {
    return runForValues()
      ...
      // return new Promise((resolve,reject) => {
         checkElement()
        //});
        //console.log("checked bp", results);
   // }
            )
        .then((results) => {
         clickkdbtnsave();
        ...
        .catch((error) => {
        // Handle errors from other functions and continue the chain
