// ==UserScript==
// @name         amazonAutoPublishClickv5.97 uk version save
// @namespace    only to translate; won't click kd for all situations;extra button
// @version      2024-06-26
    ...
    mutations.forEach((mutation) => {
        if (mutation.attributeName === "style") {
            ...
            if (newStyle === "hidden") {
                ...
                //HardwarePlatform
                                setTimeout(()=>{
                    inputElement.value = 'HardwarePlatform';
                    inputElement.dispatchEvent(event);
                    setTimeout(() => {
                        let suggestionItem = document.querySelectorAll(".el-cascader__suggestion-item");
                        if (suggestionItem) {
                            console.log("Suggestion item found:", suggestionItem);
                            suggestionItem[0].click();
                            setTimeout(() => {feibitianbuttonclick();},300);
                            setTimeout(() => {kkinputdata3();}, 500);
                        } else {
                            console.error("Suggestion item not found");
                        }
                    }, 500);
...
ifnostylechange.addEventListener('click', funcifnostylechange);
